...
class SequentialNetwork(nn.Module):
    ...
    def reset_net(self, index_list = None): 
        if index_list is None:
            for layer in self.networks: #重置全部参数
                if hasattr(layer, "reset_parameters"):
                    layer.reset_parameters()
        else:
            assert type(index_list) == list

            for index in index_list: #重置特定层的参数
                layer = self.networks[index]
                if hasattr(layer, "reset_parameters"):
                    self.networks[index].reset_parameters()
...
def get_old_network(param_shape, deconv=False):
    ...
    deconv: boolean , whether conv net
...
class MLPNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.networks.extend([final_network, out_act_cls()])
        self.networks = nn.Sequential(*self.networks) #(input,fn,200,fn,200,fn,200,fn,200,fn,out,fn), index=2,4,6,8
...
class GaussianPolicyNetwork(BasePolicyNetwork):
    ...
    def reset_net(self, index_list = None): 
        self.networks.reset_net(index_list)
