...
class MBPOTrainer(BaseTrainer):
    ...
    def warmup(self):
        ...
        for step in tqdm(range(self.warmup_timesteps)):
            ...
            self.env_buffer.add_transition(obs, action, next_obs, reward, done)
            obs = next_obs
            if done:
                obs = self.train_env.reset()
    ...
    def train(self):
        ...
        for epoch in trange(self.max_epoch, colour='blue', desc='outer loop'): # if system is windows, add ascii=True to tqdm parameters to avoid powershell bugs
            ...
            for env_step in trange(self.num_env_steps_per_epoch, colour='green', desc='inner loop'):
                ...
                self.post_step(tot_env_steps, self.return_save_path) #评估策略，进行记录
                self.post_iter(log_infos, tot_env_steps)
                
                if self.save_frequency > 0 and tot_env_steps % self.save_frequency == 0:
                    #self.save_model(self.transition_model.state_dict(), self.save_path + "{}.pth".format(tot_env_steps))
                    self.transition_model.save_model(self.model_save_path, tot_env_steps)

                if self.reset_frequency > 0 and tot_env_steps % self.reset_frequency == 0: #reset model
                    
                    self.transition_model.reset_model(index_list = self.reset_layers) #只重置最后两个hidden的参数
                
                if self.reset_sac_frequency > 0 and tot_env_steps % self.reset_sac_frequency == 0: #reset sac agent
    ...
    #     torch.save(model.state_dict(),path)
        #torch.save(self.transition_model.state_dict(), self.save_path+"{}.pth".format())
