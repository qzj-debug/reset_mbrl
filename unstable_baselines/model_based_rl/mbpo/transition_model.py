...
class TransitionModel:
    ...
    def save_model(self, model_save_path, info):
        save_dir = model_save_path
        model_save_dir = save_dir+"hopper_step_{}_".format(info)
        
        for network_name, network in self.networks.items():
            save_path = model_save_dir + network_name + ".pt"
            torch.save(network.state_dict(), save_path)
        
    def reset_model(self, index_list=None): #重置一下模型的参数
        self.model.reset_parameter(index_list)
    ...
    #     model_save_dir = os.path.join(save_dir, "step_{}".format(info))
    #     print("model_save_dir:"+str(model_save_dir))
